import { Flex, Heading, useColorModeValue, Image, Avatar, Text } from '@chakra-ui/react';
...
const Features = () => {
    ...
    const avatarBg = useColorModeValue('green.700', 'green.500');
    const textColor = useColorModeValue('blackAlpha.900', 'white');

    return (
        <Flex alignItems='center' py='6rem' px='3rem' flexDir='column' bgGradient={useColorModeValue('linear(to-r, green.100, green.50)', 'linear(to-tl, gray.800, green.900)')} id='features'>
            <Heading mb={{ base: '2rem', md: '5rem' }} textAlign='center' >Our Features</Heading>
                ...
                <Flex display={{ base: 'none', md: 'flex' }} width={{ base: '100%', md: '50%' }} flexDir='column' gap={5} alignItems='center' justifyContent='center'>
                    { featureData.map(data => (
                        <Flex key={data.id} className={classes.featurebox} gap={5} bgColor={selected === data.id ? 'white': ''} py={5} px={8} borderRadius={10} boxShadow={selected === data.id ? 'md': ''} onClick={() => setSelected(data.id)} cursor='pointer'>
                            <Avatar name={data.id.toString()} bgColor={avatarBg} size={{ base: 'sm', md: 'md' }}></Avatar>
                            <Flex flexDir='column' gap={2}>
                                <Heading color={selected === data.id ? 'blackAlpha.900' : textColor} size={{ base: 'sm', md: 'md' }}>{data.name}</Heading>
                                <Text fontSize={{ base: 'sm', md: 'md' }} color={selected === data.id ? 'blackAlpha.900' : textColor} >{data.desc}</Text>
                            </Flex>
                        </Flex>
                    ))}
                </Flex>
